...
// .wrangler/tmp/bundle-jhBTVV/checked-fetch.js
...
// .wrangler/tmp/pages-nQ3r4M/functionsWorker-0.6203588100565962.mjs
...
__name2(fetchLeetCode, "fetchLeetCode");
...
async function onRequestGet3({ params }) {
  const { username } = params;
  if (!username) {
    return errorResponse("Username is required", 400);
  }
...
async function onRequestGet4({ params }) {
  const { username } = params;
  if (!username) {
    return errorResponse("Username is required", 400);
  }
...
async function onRequestGet5({ params }) {
  const { username } = params;
  if (!username) {
    return errorResponse("Username is required", 400);
  }
  try {
    ...
    const getCount = /* @__PURE__ */ __name2((arr, diff) => arr.find((x) => x.difficulty === diff)?.count || 0, "getCount");
    return jsonResponse({
...
async function onRequestGet6({ params, request }) {
  const { username } = params;
...
// .wrangler/tmp/pages-nQ3r4M/fmnrofcsj9p.js
...
// .wrangler/tmp/bundle-jhBTVV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jhBTVV/middleware-loader.entry.ts
